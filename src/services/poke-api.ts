...
export async function fetchPokemon(pokeInput: PokemonInfo): Promise<Pokemon> {
  ...
  const pokemon: Pokemon = {
    ...
    ability: {
      name: mapNames(__ability.names, __ability.name),
      effectText: mapEffectText(__ability.effect_entries),
      flavorText: mapFlavorText(__ability.flavor_text_entries),
    },
    evs: pokeInput.evs,
    ivs: pokeInput.ivs,
    moves: _moves.map((m) => ({
...
function mapEffectText(list: VerboseEffect[]): EffectText {
  return list.reduce<EffectText>((acc, item) => {
    acc[item.language.name as Language] = {
      effect: item.effect,
      sortEffect: item.short_effect,
    }
    return acc
  }, {})
}
