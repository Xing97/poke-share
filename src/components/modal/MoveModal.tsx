...
export default function MoveModal({ move }: Props): JSX.Element {
  ...
  const pp =
    move.pastValues.find((pv) => isBeforeGame(pv.game, game) && pv.pp != null)?.pp ?? move.pp
  const power =
    move.pastValues.find((pv) => isBeforeGame(pv.game, game) && pv.power != null)?.power ??
    move.power
  const accuracy =
    move.pastValues.find((pv) => isBeforeGame(pv.game, game) && pv.accuracy != null)?.accuracy ??
    move.accuracy
  const type =
    move.pastValues.find((pv) => isBeforeGame(pv.game, game) && pv.type != null)?.type ?? move.type
  const effectText =
    move.pastValues.find((pv) => isBeforeGame(pv.game, game) && pv.effectText != null)
